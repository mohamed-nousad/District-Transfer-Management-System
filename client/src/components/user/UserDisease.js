...
const UserDisease = ({ user }) => {
  ...
  const [editingDisease, setEditingDisease] = useState(null); // State to hold the disease being edited
  ...
  // Handle form submission (add or edit disease)
  const onFinish = async (values) => {
    try {
      setLoading(true);
      const token = localStorage.getItem("token");
      const url = editingDisease
        ? `${process.env.REACT_APP_API_URL}/user/disease/${editingDisease._id}` // URL for editing disease
        : `${process.env.REACT_APP_API_URL}/user/disease`; // URL for adding disease
      const method = editingDisease ? 'put' : 'post'; // HTTP method based on if we are editing

      const response = await axios[method](url, { ...values, userId: user._id }, {
        headers: { Authorization: `Bearer ${token}` },
      });

      message.success(response.data.message || (editingDisease ? "Disease updated successfully" : "Disease added successfully"));
      form.resetFields();
      setEditingDisease(null); // Clear editing state
      fetchDiseases();
    } catch (error) {
      message.error(
  ...
  const deleteDisease = async (id) => {
    try {
      ...
      const token = localStorage.getItem("token");
      await axios.delete(
        `${process.env.REACT_APP_API_URL}/user/disease/${id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      message.error(
        error.response?.data?.error || "Failed to delete disease"
      );
    } finally {
      setLoading(false);
    }
  ...
  // Handle editing a disease
  const handleEdit = (record) => {
    setEditingDisease(record); // Set the disease to be edited
    form.setFieldsValue({
      type: record.type,
      are_you_taking_treatment: record.are_you_taking_treatment.toString(),
      treatment_date: record.treatment_date ? moment(record.treatment_date) : null,
    });
  };
      ...
      <Form form={form} layout="vertical" onFinish={onFinish}>
        ...
        {checkboxChecked ? (
          ...
          <Button type="primary" htmlType="submit" block disabled={loading}>
            {loading ? <Spin /> : editingDisease ? "Update" : "Save"}
          </Button>
        )}
      </Form>
